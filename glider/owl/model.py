...
from .config import Owlv2Config, Owlv2TextConfig, Owlv2VisionConfig


if is_vision_available():
    from transformers.image_transforms import center_to_corners_format

from ..modeling.layers import CrossFuser
...
class Owlv2SdpaAttention(Owlv2Attention):
    def __init__(self, config: Owlv2Config) -> None:
        super().__init__(config)
        self.attention_probs_dropout_prob = config.attention_dropout
...
Owl_ATTENTION_CLASSES = {
    "eager": Owlv2Attention,
    "sdpa": Owlv2SdpaAttention,
}
...
class Owlv2EncoderLayer(nn.Module):
    def __init__(self, config: Owlv2Config, vision_encoder=False):
        super().__init__()
        self.embed_dim = config.hidden_size
        if vision_encoder:
            self.self_attn = Owlv2SdpaAttention(config)
        else:
...
class Owlv2Encoder(nn.Module):
    ...
    def __init__(self, config: Owlv2Config, vision_encoder=False):
        super().__init__()
        self.layers = nn.ModuleList([Owlv2EncoderLayer(config, vision_encoder) for _ in range(config.num_hidden_layers)])
...
class Owlv2VisionTransformer(nn.Module):
    def __init__(self, config: Owlv2VisionConfig):
        ...
        self.encoder = Owlv2Encoder(config, True)
    ...
    def forward(
        ...
        if inputs_embeds is not None:
            hidden_states = self.pre_layernorm(inputs_embeds)
        else:
        ...
        last_hidden_state = encoder_outputs[0]
        if num_fused_tokens:
            last_hidden_state = last_hidden_state[:, num_fused_tokens:, :]
...
class Owlv2VisionModel(Owlv2PreTrainedModel):
    ...
    def get_embeddings(self, pixel_values: torch.FloatTensor):
        expected_input_dtype = self.vision_model.embeddings.patch_embedding.weight.dtype
        pixel_values = pixel_values.to(expected_input_dtype)
    ...
    def forward(
        ...
        return self.vision_model(
            ...
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
            num_fused_tokens=num_fused_tokens,
        )
...
class Owlv2Model(Owlv2PreTrainedModel):
    ...
    def forward(
        ...
        batch_size = pixel_values.shape[0]
        max_text_queries = text_embeds.shape[0] // batch_size

        if self.config.deep_fusion:
            text_embeds_ = self.text_vision_projector(text_embeds).reshape(batch_size, max_text_queries, text_embeds.shape[-1])
            input_ids = input_ids.reshape(batch_size, max_text_queries, input_ids.shape[-1])
            query_mask = input_ids[..., 0] > 0
            query_mask_expanded = query_mask.unsqueeze(-1).expand(text_embeds_.size()).float()
            if self.config.fusion_strategy=='mean':
                merged_text_embeds = torch.sum(text_embeds_ * query_mask_expanded, 1) / torch.clamp(query_mask_expanded.sum(1), min=1e-9).unsqueeze(1)
                num_fused_tokens = 1
            elif self.config.fusion_strategy=='full':
                merged_text_embeds = text_embeds_ * query_mask_expanded
                num_fused_tokens = text_embeds_.shape[1]

            image_input_embeds = self.vision_model.get_embeddings(pixel_values)
            inputs_embeds = torch.cat([merged_text_embeds, image_input_embeds], dim=1)
            vision_outputs = self.vision_model(inputs_embeds=inputs_embeds, output_attentions=output_attentions,
                                                        output_hidden_states=output_hidden_states, num_fused_tokens=num_fused_tokens)
        else:
            vision_outputs = self.vision_model(pixel_values, output_attentions=output_attentions,
                                                        output_hidden_states=output_hidden_states)
...
class Owlv2ForObjectDetection(Owlv2PreTrainedModel):
    ...
    def forward(
        ...
        batch_size, num_patches, hidden_dim = feature_map.shape
        # Reshape from [batch_size * max_text_queries, hidden_dim] -> [batch_size, max_text_queries, hidden_dim]
        max_text_queries = input_ids.shape[0] // batch_size
        # If first token is 0, then this is a padded query [batch_size, num_queries].
        input_ids = input_ids.reshape(batch_size, max_text_queries, input_ids.shape[-1])
        query_embeds = query_embeds.reshape(batch_size, max_text_queries, -1)
        query_mask = input_ids[..., 0] > 0

        if self.config.post_fusion_schema:
            query_embeds, feature_map = self.features_enhancement(query_embeds, feature_map)
        ...
        image_feats = torch.reshape(feature_map, (batch_size, num_patches_, num_patches_, hidden_dim))

        # Predict object classes [batch_size, num_patches, num_queries+1]
        (pred_logits, class_embeds) = self.class_predictor(feature_map, query_embeds, query_mask)

        # Predict objectness
        objectness_logits = self.objectness_predictor(feature_map)

        # Predict object boxes
        pred_boxes = self.box_predictor(feature_map, image_feats)
