...
class VitDetEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor) -> torch.Tensor:
        ...
        self.patch_embeddings(pixel_values)
...
class VitDetSelfAttention(nn.Module):
    ...
    def __init__(self, config, input_size=None):
        ...
        self.all_head_size = num_heads * head_dim
    ...
    def forward(self, hidden_state, output_attentions=False):
        ...
        keys = self.transpose_for_scores(self.key(hidden_state))
        values = self.transpose_for_scores(self.value(hidden_state))
...
class VitDetSdpaSelfAttention(VitDetSelfAttention):
    def __init__(self, config: VitDetConfig) -> None:
        super().__init__(config)
        self.attention_probs_dropout_prob = config.attention_probs_dropout_prob

    def forward(
        self, hidden_state, output_attentions: bool = False
        ...
        hidden_state = torch.nn.functional.scaled_dot_product_attention(
            queries,
            keys,
            values,
            None,
...
class VitDetAttention(nn.Module):
    def __init__(self, config: VitDetConfig, input_size=None) -> None:
        super().__init__()
        self.attention = VitDetSelfAttention(config, input_size)
...
class VitDetSdpaAttention(VitDetAttention):
    def __init__(self, config: VitDetConfig) -> None:
        super().__init__(config)
        self.attention = VitDetSdpaSelfAttention(config)
...
class VitDetLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output = self.intermidiate(layer_output)
        hidden_states = self.output(layer_output, hidden_states)
...
class VitDetPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["VitDetEmbeddings", "VitDetLayer"]
    _supports_sdpa = True
